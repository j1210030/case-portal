...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Calculate backlog group data 
            backlog_data = Case.objects.filter(~Q(status__in=['duplicate','routed','forwarded','solution_offered'])).\
                                values('gcase_user_id').annotate(case_count=Count('*')).order_by('gcase_user_id')
            ...
            for user in users:
                ...
                if guser_id == 2 or guser_id == 3 or guser_id == 5 or guser_id == 6 or guser_id == 7 or guser_id == 8 or guser_id == 9 or guser_id == 10 or guser_id == 17 or guser_id == 18 or guser_id == 25:
                    self.create_data(guser_id,dt,incoming_data,incoming_partner_data,so_data,so_partner_data, backlog_data) 
                    
        except Exception, ex:
             log.exception("SQL Error Encountered in jd search. " + str(ex))
        
    def create_data(self,user_id,week,incoming_data, incoming_partner_data, so_data, so_partner_data, backlog_data):
        ...
        if backlog_data is not None and len(backlog_data) > 0:
            for item in backlog_data:
                if item['gcase_user_id'] == user_id:
                    pw_backlog = item['case_count']
                    break
